...
public class Enemy4 extends Entity{
	...
	public int nextTime = Entity.rand.nextInt(300 - 180) + 180;
	
	private BufferedImage[] spriteEnemy4;
	private BufferedImage[] spriteEnemy4Ghost;
	
	private int index = 0, maxIndex = 1, frames = 0, maxFrames = 60, indexD = 0, maxDIndex = 1, framesD = 0, maxDFrames = 25;
	
	public Enemy4(int x, int y, int width, int height,int speed, BufferedImage sprite) {
		super(x, y, width, height,speed, sprite);
		spriteEnemy4 = new BufferedImage[2];
		spriteEnemy4[0] = Game.spritesheet.getSprite(96, 48, 16, 16);
		spriteEnemy4[1] = Game.spritesheet.getSprite(112, 48, 16, 16);
		
		spriteEnemy4Ghost = new BufferedImage[2];
		spriteEnemy4Ghost[0] = Game.spritesheet.getSprite(96, 80, 16, 16);
		spriteEnemy4Ghost[1] = Game.spritesheet.getSprite(112, 80, 16, 16);
	}
	...
	public void render(Graphics g) {
		if(ghostMode == false) {
			g.drawImage(spriteEnemy4[index], this.getX() - Camera.x, this.getY() - Camera.y, null);
		}else {
			g.drawImage(spriteEnemy4Ghost[index], this.getX() - Camera.x, this.getY() - Camera.y, null);
		}
	}
